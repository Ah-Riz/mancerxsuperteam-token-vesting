...
export const campaigns = pgTable(
  "campaigns",
  {
    ...
    campaignId: bigint("campaign_id", { mode: "string" }).notNull(),
    merkleRoot: text("merkle_root").notNull(),
    leafCount: integer("leaf_count").notNull(),
    totalSupply: bigint("total_supply", { mode: "string" }).notNull(),
    totalClaimed: bigint("total_claimed", { mode: "string" }).notNull().default("0"),
    ...
    cancelledAt: bigint("cancelled_at", { mode: "string" }),
    paused: boolean("paused").notNull().default(false),
    createdAt: bigint("created_at", { mode: "string" }).notNull(),
...
export const leaves = pgTable(
  "leaves",
  {
    ...
    startTime: bigint("start_time", { mode: "string" }).notNull(),
    cliffTime: bigint("cliff_time", { mode: "string" }).notNull(),
    endTime: bigint("end_time", { mode: "string" }).notNull(),
...
export const claimEvents = pgTable(
  "claim_events",
  {
    ...
    leafIndex: integer("leaf_index").notNull(),
    amount: bigint("amount", { mode: "string" }).notNull(),
    totalClaimedByUser: bigint("total_claimed_by_user", { mode: "string" }).notNull(),
    totalClaimedOverall: bigint("total_claimed_overall", { mode: "string" }).notNull(),
    milestoneIdx: smallint("milestone_idx"),
    signature: text("signature").notNull().unique(),
    slot: bigint("slot", { mode: "string" }).notNull(),
    blockTime: bigint("block_time", { mode: "string" }).notNull(),
  },
  (table) => [
